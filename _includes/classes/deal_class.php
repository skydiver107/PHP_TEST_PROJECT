...
class deal{
	...
	public function insert_new(){
		...
		$finance_cost = $_SESSION["finance_cost"]; //finance_cost
		 ...
		 $finance_gross = array_sum($finance_sale) - array_sum($finance_cost);
		 $Json_misc_id = json_encode($misc_id);
		 $data['dealer_check'] = $data['amount_financed'] - $data['discount'];
		...
		foreach($misc_id as $key=>$value)
		 {
			...
			$data_update =array("misc_id"=>$Json_misc_id,"finance_cost"=>$Json_finance_cost,"finance_sale"=>$Json_finance_sale,"finance_cost"=>$Json_finance_cost,"finance_gross"=>$finance_gross);
	...
	public function update_one(){
		...
		$data['locked'] = $this->simplePostData('locked');
		$data['pending_documents'] = $this->simplePostData('pending_documents');
		$data['misc_id'] = $Json_misc_id;
		$data['finance_cost'] = $Json_finance_cost;
		$data['finance_sale'] = $Json_finance_sale;
		$data['finance_gross'] = array_sum($finance_sale) - array_sum($finance_cost);
		unset($data['id']);
		unset($data['salespeople']);
		unset($data['pickuppayments']);		
