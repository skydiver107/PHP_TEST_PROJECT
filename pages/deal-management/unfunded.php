...
$_SESSION['FT']['current_page'] = 'unfunded';
...
if(isset($_GET['dropdown'])) {
    ...
    echo $controller->getUnfundedTableList($_GET, $_SESSION['FT']['company_id']);
} else {
    $twig->addGlobal('page_title', 'Unfunded Deals');
    $controller = new dealcontroller();
    //$output = $controller->invoke('unfunded');
    $company_id = $_SESSION['FT']['company_id'];
    $str_where = "company_id = $company_id AND funded = 0 ";
    ...
    $_SESSION["current_page"] = "unfunded";
    ...
    echo $twig->render('deal-management/tpl.unfunded.twig');
}
